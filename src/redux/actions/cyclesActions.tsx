...
export const applicantApplyCycle = async (cycleId: any) => {
  try {
    const response = await axios.post("/", {
      query: `
            mutation applyCycle($input: applyCycleInput!) {
              applyCycle(input: $input) {
                message
              }
            }
          `,
...
export const applicantGetCyclesApplications = async () => {
  try {
    const response = await axios.post(
      "/graphql",
      {
...
export const getApplicantsStageHistory = async (applicantId) => {
  try {
    const response = await axios.post("/graphql", {
      query: `
        query getStageHistoryByApplicant($applicantId: ID!) {
    ...
    if (response.data?.data?.getStageHistoryByApplicant) {
      return response.data.data.getStageHistoryByApplicant;
    } else {
      return null;
    }
  } catch (error) {
    console.error("Failed to retrieve applicant stage history:", error);
    throw error;
  }
};
