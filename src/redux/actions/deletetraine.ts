...
export const fetchtraine = ({ page,itemsPerPage,  All }:any) => {
    return async (dispatch: Dispatch<fetchact>) => {
        try {
            ...
            .then((res)=> {
                if (res.data.data){toast.success("traine-applicants fetched successfully") ;dispatch({type: fetchtrainesss.fetchtraines_success,data:res.data.data.allTrainees})};
                if (res.data.errors) {
                     var mess
                res.data.errors.map((b:any)=>{ mess =b.message})
