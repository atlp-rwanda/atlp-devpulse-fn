...
          />
      ...
      <div className="overflow-y-auto pt-12">
        {roleName === 'applicant' ? (
          <SidebarSection items={applicantSidebarItems} title="Applicant Section" expanded={expanded} open={openSections.general} toggle={() => toggleSection('general')} />
        ) : (
          <>
            <SidebarSection items={generalSectionItems} title="General" expanded={expanded} open={openSections.general} toggle={() => toggleSection('general')} />
            <SidebarSection items={managementSectionItems} title="Management" expanded={expanded} open={openSections.management} toggle={() => toggleSection('management')} />
            <SidebarSection items={applicationsSectionItems} title="Applications and Programs" expanded={expanded} open={openSections.applications} toggle={() => toggleSection('applications')} />
            <SidebarSection items={performanceSectionItems} title="Performance and Grading" expanded={expanded} open={openSections.performance} toggle={() => toggleSection('performance')} />
            <SidebarSection items={adminSectionItems} title="Admin" expanded={expanded} open={openSections.admin} toggle={() => toggleSection('admin')} />
          </>
        )}
    ...
    {open && (
      <ul className="pl-4 mt-2">
        {items.map((item, index) => (
          <li key={index} className="flex items-center text-white text-base dark:text-white">
            <Link to={item.path} className="is-active focus:text-green-600 p-1 flex items-center leading-3 cursor-pointer font-semibold hover:font-bold">
