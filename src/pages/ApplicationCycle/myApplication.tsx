    ...
    if (!application) {
      ...
      const fetchAttributesAndStages = async () => {
        try {
          setIsLoading(true);
          if (application?._id) {
            const attributesResponse = await getCyclesApplicationAttributes(
              application._id
            );
          ...
          <div className="flex flex-col sm:flex-row border-b border-gray-300 dark:border-gray-600 pb-4">
            <div className="flex-1 pr-4 mb-4 sm:mb-0">
              ...
              <ul className="list-disc list-inside space-y-2 text-gray-800 dark:text-gray-300">
                {application?.firstName && (
                  <li>
                    <strong>Firstname:</strong> {application.firstName}
                  </li>
                )}
                {application?.lastName && (
                  <li>
                    <strong>Lastname:</strong> {application.lastName}
                  </li>
                )}
                {application?.email && (
                  <li>
            ...
            <div className="flex-1 pl-4">
              ...
              <ul className="list-disc list-inside space-y-2 text-gray-800 dark:text-gray-300">
                {attributes?.province && (
                  <li>
                    <strong>Province:</strong> {attributes.province}
                  </li>
                )}
                {attributes?.district && (
                  <li>
                    <strong>District:</strong> {attributes.district}
                  </li>
                )}
                {attributes?.sector && (
                  <li>
                    <strong>Sector:</strong> {attributes.sector}
                  </li>
                )}
                {attributes?.Address && (
                  <li>
                    <strong>Address:</strong> {attributes.Address}
                  </li>
                )}
              </ul>
            </div>
          ...
          <section>
            ...
            <div className="bg-gray-100 dark:bg-[#262E3D] p-4 rounded-lg mt-4 shadow">
              ...
              {!stages?.dismissed &&
                !stages?.interview &&
                !stages?.shortlist && (
                  <p className="text-red-500">No stages history found</p>
                )}
              <ul className="space-y-6 list-disc list-inside marker:text-[#56C870] dark:marker:text-green-400 bg-red">
                {stages?.dismissed && (
                  <li className="bg-red-500 text-white p-6 rounded-lg shadow-md">
                    <strong className="font-semibold text-xl mb-3">
                      Rejected
                    </strong>

                    {stages.dismissed?.status && (
                      <p>
                        <strong>Status:</strong> {stages.dismissed.status}
                      </p>
                    )}
                    ...
                    {stages.dismissed?.comments && (
                      <p>
                        <strong>Comments:</strong> {stages.dismissed.comments}
                      </p>
                    )}
                ...
                {stages?.admitted && (
                  <li className="">
                    ...
                    {stages.admitted?.status && (
                      <p>
                        <b>Status:</b> {stages.admitted.status}
                      </p>
                    )}
                    {stages.admitted?.stageDismissedFrom && (
                      <p>
                        <b>Score:</b> {stages.admitted.stageDismissedFrom}
                      </p>
                    )}
                    {stages.admitted?.comments && (
                      <p>
                        <b>Comments:</b> {stages.admitted.comments}
                      </p>
                    )}
                    {stages.admitted?.createdAt && (
                      <p>
                        <b>Date:</b> {formatDate(stages.admitted.createdAt)}
                      </p>
                    )}
                  </li>
                )}
                {stages?.interview && (
                  <li>
                    ...
                    {stages.interview?.interviewScore && (
                      <p>
                        <b>Score:</b> {stages.interview.interviewScore}
                      </p>
                    )}
                    {stages.interview?.status && (
                      <p>
                        <b>Status:</b> {stages.interview.status}
                      </p>
                    )}
                    {stages.interview?.comments && (
                      <p>
                        <b>Comments:</b> {stages.interview.comments}
                      </p>
                    )}
                    {stages.interview?.createdAt && (
                      <p>
                        <b>Date:</b> {formatDate(stages.interview.createdAt)}
                      </p>
                    )}
                  </li>
                )}
                {stages?.technical && (
                  <li>
                    ...
                    {stages.technical?.score && (
                      <p>
                        <b>Score:</b> {stages.technical.score}%
                      </p>
                    )}
                    {stages.technical?.status && (
                      <p>
                        <b>Status:</b> {stages.technical.status}
                      </p>
                    )}
                    {stages.technical?.comments && (
                      <p>
                        <b>Comments:</b> {stages.technical.comments}
                      </p>
                    )}
                    {stages.technical?.createdAt && (
                      <p>
                        <b>Date:</b> {formatDate(stages.technical.createdAt)}
                      </p>
                    )}
                  </li>
                )}
                {stages?.shortlist && (
                  <li>
                    ...
                    {stages.shortlist?.status && (
                      <p>
                        <b>Status:</b> {stages.shortlist.status}
                      </p>
                    )}
                    {stages.shortlist?.comments && (
                      <p>
                        <b>Comments:</b> {stages.shortlist.comments}
                      </p>
                    )}
                    {stages.shortlist?.createdAt && (
                      <p>
                        <b>Date:</b> {formatDate(stages.shortlist.createdAt)}
                      </p>
                    )}
                  </li>
                )}
