...
import {
  ...
  getAllprograms,
  sendBulkyEmail,
} from "../../redux/actions/filterProgramActions";
...
const FilterProgram = (props: any) => {
  ...
  // All programs in DB
  const { allfilteredPrograms, updateTraineeStatus, count } = props;
  const programList = allfilteredPrograms?.data;
  const handleNullProgramList = programList === undefined ? [] : programList;

  const [itemsPerPage, setitemsPerPage] = useState(10);

  const nonNullProgram = handleNullProgramList?.filter((value) => {
    return value !== null;
  });

  const nonDeletedProgram = nonNullProgram?.filter((value) => {
    return value.program_id.delete_at == false;
  });
  ...
  const COLS = [
    {
      ...
      Cell: ({ row }: any) => {
        return (
          <div>
            <span>{row.original.program_id.programName} </span>
            <span className="semi-md-col:hidden">
              {row.original.program_id.programName}
            </span>
          </div>
        );
      },
    ...
    },
    {
      ...
      accessor: "program_id.mainObjective",
    },
    {
      Header: "Deleted",
      accessor: "",
      Cell: ({ row }: any) => {
        return <div>{row.original.program_id.delete_at.toString()}</div>;
      },
    },
    {
      Header: "Mode Of Execution",
      accessor: "program_id.modeOfExecution",
    },
    {
      Header: "Requirements",
      accessor: "program_id.requirements",
      Cell: ({ row }: any) => {
    ...
    },
    {
      ...
      Cell: ({ row }: any) => {
        return (
          <select
            defaultValue={row.original.program_id.status}
            id="status"
            ...
            onChange={(e) => {
              traineeStatusUpdate(
                row.original.program_id._id,
                e.target.value,
                row.original.program_id.cycle_id.id
              );
            }}
          >
  ...
  const data = useMemo(() => nonDeletedProgram, [allfilteredPrograms]);
  useEffect(() => {
    ...
    props.getAllFilteredPrograms(data23);
    setPageData((prevstate) => ({
      ...prevstate,
      isLoading: false,
      rowData: data,
      totalPrograms: count.message,
    }));
  ...
  const rowsSelected = selectedFlatRows.map(
    (row) => row.original.program_id.email
  );
            ...
            <div className="">
              <Select
                ...
                options={[
                  ...
                  { value: "programDescription", label: "Program Description" },
