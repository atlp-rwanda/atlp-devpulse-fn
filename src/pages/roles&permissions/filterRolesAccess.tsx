...
import {
    getAllFilteredRolesAccess,
    getAllRoles,
    sendBulkyEmail
} from "../../redux/actions/filterRolesAccess"
...
const FilterRole = (props: any) => {
  ...
  // All Roles in DB
  const { allfilteredRoles, updateTraineeStatus, count } = props;
  const RoleList = allfilteredRoles?.data;
  const handleNullRoleList = RoleList === undefined ? [] : RoleList;

  const [itemsPerPage, setitemsPerPage] = useState(10);

  const nonNullRole = handleNullRoleList?.filter((value) => {
    return value !== null;
  });

  const nonDeletedRole = nonNullRole?.filter((value) => {
    return value.Role_id.delete_at == false;
  });
  ...
  const COLS = [
    {
      ...
      Cell: ({ row }: any) => {
        return (
          <div>
            <span>{row.original.role_id.roleName} </span>
            <span className="semi-md-col:hidden">
              {row.original.role_id.roleName}
            </span>
          </div>
        );
      },
    ...
    },
    {
      ...
      accessor: "",
      Cell: ({ row }: any) => {
        return <div>{row.original.role_id.delete_at.toString()}</div>;
      },
    },
    {
      Header: "Description",
      accessor: "role_id.description",
    },
    {
      Header: "Status",
      accessor: "",
      Cell: ({ row }: any) => {
        return (
          <select
            defaultValue={row.original.role_id.status}
            id="status"
            ...
            onChange={(e) => {
              traineeStatusUpdate(
                row.original.role_id._id,
                e.target.value,
                row.original.role_id.cycle_id.id
              );
            }}
          >
  ...
  const data = useMemo(() => nonDeletedRole, [allfilteredRoles]);
  useEffect(() => {
    ...
    props.getAllFilteredRolesAccess(data23);
    setPageData((prevstate) => ({
      ...prevstate,
      isLoading: false,
      rowData: data,
      totalRoles: count.message,
    }));
  ...
  const rowsSelected = selectedFlatRows.map(
    (row) => row.original.role_id.email
  );
...
const mapState = (state: any) => ({
  allfilteredRoles: state.FilterRole,
  errors: state.errors,
  count: state.count,
});

export default connect(mapState, {
  getAllFilteredRolesAccess: getAllFilteredRolesAccess,
