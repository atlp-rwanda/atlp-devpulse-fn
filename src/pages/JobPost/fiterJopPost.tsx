...
const FilterJobPost = (props: any) => {
  ...
  const nonDeletedJobPost = nonNullJobPost?.filter((value) => {
  ...
  const COLS = [
    {
      ...
      Cell: ({ row }: any) => {
        return (
          <div>
            <span>{row.original.jobPost_id.jobTitle} </span>
            <span className="semi-md-col:hidden">
              {row.original.jobPost_id.jobTitle}
            </span>
          </div>
        );
      },
  ...
  useEffect(() => {
    ...
    props.getAllFilteredJobPosts(data23);
    setPageData((prevstate) => ({
      ...prevstate,
      isLoading: false,
      rowData: data,
